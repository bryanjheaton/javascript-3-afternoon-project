...
var evenNumbers = mixedNumbers.filter(nbr => (nbr % 2 === 0));
...
//Code Here
let totalPopulation   = populations.reduce((total, current, index, array) => total + current)
...
//Code Here
let myStrongest = monstersInYourPocket.filter((element, index, array) => element.CP > 200) 
...
let bobsTotal = purchases.filter((element) => element.owner === "Bob").reduce((total, item) => total + item.price, 0)
